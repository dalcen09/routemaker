      ...
      <div className="flex-1 flex min-h-0">
        ...
        <div className="flex-1 relative min-w-0">
          {loading ? (
            ...
            <CustomersMap customers={customers} selected={selected} onToggle={toggleSelected} />
          )}
        </div>
        ...
        {selected.size > 0 && (
          <div className="hidden md:flex flex-col w-72 shrink-0 bg-white border-l border-slate-200 shadow-lg">
            <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
              ...
              <button onClick={clearSelection} className="text-xs text-slate-400 hover:text-slate-600 transition-colors">全て解除</button>
            </div>
              ...
              {selectedCustomers.map((c) => (
                <div key={c.id} className="flex items-start gap-3 px-4 py-2.5 hover:bg-slate-50 group">
                  <div className="flex-1 min-w-0">
                    ...
                    {c.company && <div className="text-xs text-slate-400 truncate mt-0.5">{c.company}</div>}
                    {c.address && <div className="text-xs text-slate-400 truncate mt-0.5">{c.address}</div>}
                  </div>
                  <button onClick={() => removeSelected(c.id)} className="shrink-0 mt-0.5 w-5 h-5 rounded-full text-slate-300 hover:text-red-500 hover:bg-red-50 flex items-center justify-center transition-colors opacity-0 group-hover:opacity-100" title="選択を解除">
            ...
            <div className="p-4 border-t border-slate-100">
              <button onClick={handleGenerateRoute} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2.5 rounded-xl text-sm transition-colors shadow-sm flex items-center justify-center gap-2">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9 6.75V15m6-6v8.25m.503 3.498l4.875-2.437c.381-.19.622-.58.622-1.006V4.82c0-.836-.88-1.38-1.628-1.006l-3.869 1.934c-.317.159-.69.159-1.006 0L9.503 3.252a1.125 1.125 0 00-1.006 0L3.622 5.689C3.24 5.88 3 6.27 3 6.695V19.18c0 .836.88 1.38 1.628 1.006l3.869-1.934c-.317-.159.69-.159 1.006 0l4.994 2.497c.317.158.69.158 1.006 0z" />
                </svg>
                ルートを生成
          ...
          <div className="flex-1 overflow-y-auto py-2">
            {selectedCustomers.map((c) => (
              <div key={c.id} className="flex items-start gap-3 px-4 py-2.5 hover:bg-slate-50">
                ...
                <button onClick={() => removeSelected(c.id)} className="shrink-0 mt-0.5 w-5 h-5 rounded-full text-slate-300 hover:text-red-500 hover:bg-red-50 flex items-center justify-center transition-colors" title="選択を解除">
                  <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                  </svg>
                </button>
